...
import { CreateUserType, UpdateUserType, UserInterface, UserWithDepartmentInterface } from "../types/user-type";

const apiName = "/user";

export const getUsers = async (): Promise<UserWithDepartmentInterface[]> => {
    try {
        const response: ApiResponseInterface<UserWithDepartmentInterface[]> = await axiosGet(`${apiName}/`);
