...
const ModalCreatePersonalComputer: React.FC<ModalCreatePersonalComputerProps> = ({ isOpen, onDidDismiss }) => {
	const [formData, setFormData] = useState<CreatePersonalComputerType>({ assetNo: "", invoiceNo: "", ownerName: "", serialNumber: "", detailName: "", DepartmentId: "", LocationId: "" });
	const { mutate } = useCreatePersonalComputer();

	const handleInput = (key: keyof CreatePersonalComputerType, value: string) => {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		mutate(formData);
		onDidDismiss();
		setFormData({ assetNo: "", invoiceNo: "", ownerName: "", serialNumber: "", detailName: "", DepartmentId: "", LocationId: "" });
	}

	return (
		<Modal title="Create Personal Computer" isOpen={isOpen} onDidDismiss={onDidDismiss}>
			<form onSubmit={handleSubmit}>
				<IonItem>
					<IonInput type="text" label="Serial Number" labelPlacement="floating" value={formData.serialNumber} onIonInput={(e) => handleInput("serialNumber", e.detail.value!)} required />
				</IonItem>
				<IonItem>
